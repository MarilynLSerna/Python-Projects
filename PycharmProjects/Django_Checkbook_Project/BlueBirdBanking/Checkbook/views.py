...
def create_account(request):
    ...
    if request.method == 'POST':
        if form.is_valid(): # Checks to see if the submitted form is valid and if so, saves the form
            ...
            return redirect('index') # Returns use back to the home page
...
def balance(request, pk):
    account = get_object_or_404(Account, pk=pk) # Retrieve all of that account using its primary key
